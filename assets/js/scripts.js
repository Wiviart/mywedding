...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('wish-form').addEventListener('submit', function (event) {
        ...
        commentBox.className = 'box-comment p-3 mx-2 mb-3';
        commentBox.innerHTML = `<h4>${name}</h4><p class="m-0">${content}</p>`;

        // Thêm lời chúc vào danh sách bình luận
        document.getElementById('show-comments').appendChild(commentBox);

        // Xóa nội dung form sau khi gửi
        document.getElementById('wish-form').reset();

        console.log('Dữ liệu form:', name, content);
    });
});